...
jQuery.ketchup.validation("required","Este campo es requerido.",function(g,b,c){g=b.attr("type").toLowerCase();return g=="checkbox"||g=="radio"?b.attr("checked")==true:c.length!=0}).validation("minlength","Este campo tiene un minimo de {arg1}.",function(g,b,c,d){return c.length>=+d}).validation("maxlength","Este campo debe tener un maximo de {arg1} de longitud.",function(g,b,c,d){return c.length<=+d}).validation("rangelength","This field must have a length between {arg1} and {arg2}.",function(g,
b,c,d,h){return c.length>=d&&c.length<=h}).validation("min","Debe tener por lo menos {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c>=+d}).validation("max","No puede ser mayor que {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c<=+d}).validation("range","Debe estar entre {arg1} y {arg2}.",function(g,b,c,d,h){return this.isNumber(c)&&+c>=+d&&+c<=+h}).validation("number","Debe ser un numero.",function(g,b,c){return this.isNumber(c)}).validation("digits","Must be digits.",function(g,b,c){return/^\d+$/.test(c)}).validation("email",
